...
/* ============================================================ BASE PATH — GITHUB PAGES ============================================================ */ const BASE_PATH =
  "/inessogadzi";
/* ============================================================ PROJETS ============================================================ */ const PROJECTS =
  [
...
/* ============================================================ GALERIE ============================================================ */ const PROJECT_COUNT = 108;
const IMAGE_SIZE = 155;
const STEP_X = 193;
const STEP_Y = 193;
const GALLERY_IMAGE_SCALE = 1;
/* ============================================================ SPHÈRE ============================================================ */ const SPHERE_RADIUS = 2.7;
const PROJECT_SIZE = 0.56;
const SPHERE_RELAXATION_ITERATIONS = 80;
const SEGMENTS_X = 18;
const SEGMENTS_Y = 12;
const CAMERA_FOV = 58;
/* ============================================================ TRANSITION ============================================================ */ const TRANSITION_DURATION = 1800;
/* ============================================================ COURBURE GALERIE ============================================================ */ const GALLERY_CURVE_HORIZONTAL = 0.0000000001;
const GALLERY_CURVE_VERTICAL = 0.0000000001;
/* ============================================================ GRAIN ============================================================ */ const GRAIN_CONTRAST = 1.0;
const GRAIN_SAMPLES = 3;
const GRAIN_OPACITY = 0.16;
const GRAIN_UPDATE_EVERY = 4;
/* ============================================================ CURSEUR ============================================================ */ function CursorTrail(): React.JSX.Element {
...
/* ============================================================ GRAIN AU-DESSUS DE TOUT ============================================================ */ function GrainOverlay(): React.JSX.Element | null {
...
/* ============================================================ POSITION SPHÈRE ============================================================ */ type SpherePosition =
...
/* ============================================================ RELAXATION SPHÈRE ============================================================ */ function relaxSpherePoints(): THREE.Vector3[] {
...
/* ============================================================ LATITUDE / LONGITUDE ============================================================ */ function createPositions(): SpherePosition[] {
...
/* ============================================================ POINT SPHÈRE ============================================================ */ function getSpherePoint(
  radius: number,
  latitude: number,
  longitude: number,
): THREE.Vector3 {
...
/* ============================================================ GÉOMÉTRIE COURBÉE ============================================================ */ function createCurvedGeometry(
  latitude: number,
  longitude: number,
...
/* ============================================================ GÉOMÉTRIE PLATE ============================================================ */ function createFlatGeometry(): THREE.BufferGeometry {
  const geometry = new THREE.BufferGeometry();
  const vertices: number[] = [];
  const uvs: number[] = [];
  const indices: number[] = [];
...
/* ============================================================ ROTATIONS INITIALES ============================================================ */ const LOCAL_ROTATION_X =
...
/* ============================================================ MAPPING GALERIE ============================================================ */ function getGalleryImageIndex(
  column: number,
  row: number,
  imageCount: number,
): number {
...
/* ============================================================ CELLULES GALERIE ============================================================ */ type GalleryCell =
...
/* ============================================================ CELLULES TRANSITION ============================================================ */ function createTransitionGalleryAssignments(
  width: number,
  height: number,
...
/* ============================================================ POSITION ÉCRAN GALERIE ============================================================ */ type GalleryScreenPosition =
  { x: number; y: number };
...
/* ============================================================ ABOUT ============================================================ */ export default function About(): React.JSX.Element {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const [transitioning, setTransitioning] = useState(false);
  const pathname = usePathname();
  const isEnglish = pathname.startsWith(`${BASE_PATH}/en`);
  const homePath = isEnglish ? `${BASE_PATH}/en` : BASE_PATH;
  const galleryPath = isEnglish
  ...
  useEffect(() => {
    ...
    /* ======================================================== GLOBE ======================================================== */ const globe =
      new THREE.Group();
    scene.add(globe);
    /* ======================================================== RESSOURCES ======================================================== */ const textureLoader =
    ...
    /* ======================================================== CRÉATION PROJETS ======================================================== */ const createProjects =
      async (): Promise<void> => {
    ...
    void createProjects();
    /* ======================================================== TRANSITION ======================================================== */ let isTransitioning = false;
    ...
    /* ======================================================== PIXELS -> MONDE ======================================================== */ function screenToWorld(
      x: number,
      y: number,
    ): THREE.Vector3 {
      const depth = 2;
    ...
    /* ======================================================== DÉMARRAGE TRANSITION ======================================================== */ const startTransition =
      (): void => {
        ...
        meshes.forEach((mesh, meshIndex) => {
          ...
          const safeCell = cell ?? { column: 0, row: 0 };
    ...
    /* ======================================================== ANIMATION ======================================================== */ let animationFrame = 0;
    let time = 0;
    ...
    /* ======================================================== EXPOSITION TRANSITION ======================================================== */ (
      canvas as HTMLCanvasElement & { startAboutTransition?: () => void }
  ...
  /* ========================================================== CLICK DÉCOUVRIR ========================================================== */ const handleDiscover =
    (): void => {
      const canvas = canvasRef.current;
      if (canvas === null) {
        return;
  ...
  /* ========================================================== TEXTES ========================================================== */ const title =
    isEnglish ? "Inès SOGADZI's Portfolio" : "Portfolio d' Inès SOGADZI";
  const description = isEnglish ? (
    <>
      {" "}
      It all started with fashion: a passion that gradually became an open door
      to image, design and creation. <br /> <br /> Throughout my journey, I have
      learned not only to create, but to think about ideas as a whole: from
      artistic direction to marketing, from content creation to project
      management. <br /> <br /> Today, I enjoy bridging strategy and creativity,
      always driven by the same desire: to turn a moment of attention into
      genuine engagement. <br />{" "}
    </>
  ) : (
    <>
      {" "}
      Tout a commencé avec la mode : une passion devenue peu à peu une porte
      ouverte vers l’image, le design et la création. <br /> <br /> Au fil de
      mon parcours, j’ai appris à ne plus seulement créer, mais à penser les
      idées dans leur ensemble : de la direction artistique au marketing, de la
      création de contenu à la gestion de projet. <br /> <br /> Aujourd’hui,
      j’aime faire le lien entre stratégie et création, avec toujours une même
      envie : transformer une seconde d’attention en véritable engagement.{" "}
      <br />{" "}
    </>
  );
  const discoverLabel = isEnglish ? "DISCOVER" : "DÉCOUVRIR";
  /* ========================================================== HTML ========================================================== */ return (
    <>
      {" "}
      <CursorTrail />{" "}
      <main
        className={`${spaceGrotesk.className} about-page ${transitioning ? "is-transitioning" : ""}`}
      >
        {" "}
        {/* ================================================== SCÈNE THREE ================================================== */}{" "}
        <canvas ref={canvasRef} className="three-canvas" aria-hidden="true" />{" "}
        {/* ================================================== LOGO ================================================== */}{" "}
        <a href={homePath} className="logo" aria-label="Accueil">
          {" "}
          <Image
